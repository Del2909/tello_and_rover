...
class FrontEnd(object):
    ...
    def __init__(self):
        ...
        self.fSpeed = 117 / 10  # Forward Speed in cm/s   (15cm/s) (not so repeatable-so need to test how far it goes in 10s)

        self.aSpeed = 360 / 10  # Angular Speed Degrees/s  (50d/s) (same as before but need to rotate and test)

        self.interval = 0.25 #frequency of plotting the data (s)

        self.dInterval = self.fSpeed * self.interval

        self.aInterval = self.aSpeed * self.interval
        ...
        self.map_img = np.zeros((300, 300, 3), np.uint8)
    ...
    def getKeyStroke(self):
        self.d = 0 
        if self.for_back_velocity==S:  # set forward velocity
            #self.for_back_velocity = S
            self.d = self.dInterval
            self.a=270
        elif self.for_back_velocity == -S:  # set backward velocity
            #self.for_back_velocity = -S
            self.d = -self.dInterval
            self.a=-90
        elif self.left_right_velocity == -S:  # set left velocity
            #self.left_right_velocity = -S
            self.d = self.dInterval
            self.a=-180
        elif self.left_right_velocity == S:  # set right velocity
            #self.left_right_velocity = S
            self.d = -self.dInterval
            self.a=180
            ...
            #self.up_down_velocity = -S
        elif self.yaw_velocity == -aspeed:  # set yaw counter clockwise velocity
            #self.yaw_velocity = -aspeed
            self.yaw -= self.aInterval
        elif self.yaw_velocity == aspeed:  # set yaw clockwise velocity
            #self.yaw_velocity = aspeed
            self.yaw += self.aInterval
        ...
        self.x += int(self.d * math.cos(math.radians(self.a)))

        self.y += int(self.d * math.sin(math.radians(self.a)))
    ...
    def drawPoints(self):
        for point in self.points:
            cv2.circle(self.map_img, point, 5, (0, 0, 255), cv2.FILLED)

        cv2.circle(self.map_img, self.points[-1], 8, (0, 255, 0), cv2.FILLED)

        cv2.putText(self.map_img, f'({(self.points[-1][0] - 150) / 100},{(self.points[-1][1] - 150) / 100})m',
    ...
    def landAutoAruco(self):
        ...
        if detected==1:
            ...
            if tvec[0]>50:
                ...
                command[0] = -5
                self.for_back_velocity = -5 #move back
            elif tvec[0]<-error_margin:
                command[0] = 5
                self.for_back_velocity = 5 #move front
            else:
                aligned[0]= 1
                command[0] = 0
                self.for_back_velocity = 0
            ...
            if tvec[1]>50:
                ...
                self.left_right_velocity = -5 # set left velocity
                command[1] = -5
            elif tvec[1]<-error_margin:
                self.left_right_velocity = 5 # set right velocity
                command[1] = 5
            else:
                aligned[1]=1
                command[1] = 0
    ...
    def detectAruco(self):
        ...
        z_dist=self.tello.get_distance_tof()

        #print("IDS detected:  ",ids )
        #print("height is: ",z_dist)
