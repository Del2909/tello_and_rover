...
class FrontEnd(object):
    ...
    def __init__(self):
        ...
        self.map_img = np.zeros((1000, 1000, 3), np.uint8)
    ...
    def run(self):
        ...
        while not should_stop:
            ...
            if self.points[-1][0] != self.x or self.points[-1][1] != self.y:
                self.points.append((self.y, self.y))

            self.drawPoints(self)
    ...
    def keydown(self, key):
        ...
        if key == pygame.K_UP:  # set forward velocity
            ...
            self.left_right_velocity = -aspeed
            self.d = self.dInterval
            self.a=-180
    ...
    def drawPoints(self):
        ...
        cv2.putText(self.map_img, f'({(self.points[-1][0] - 500) / 100},{(self.points[-1][1] - 500) / 100})m',
